...
final case class TimeIntervalForDate(date: LocalDate, interval: TimeInterval) {
  ...
  /** Return a new TimeIntervalForDate, corresponding to current interval in targetTimeZone If converted interval
    * straddle 2 days, return None (cannot be converted to unique TimeIntervalForDate)
    */
