...
class TimeIntervalOverlapOnlyOnMyEndSpec extends WordSpec with Matchers {
  ...
  "TimeInterval#overlapOnlyOnMyEnd" when {
    "true" should {
      "be true" in {
        true shouldBe true
      }
    ...
    "1. intervals are not connected" when {
      "1.1 passed interval is before" in {
        ...
        interval overlapOnlyOnMyEnd passed shouldBe false
      }
    ...
    "4. passed interval overlap the 'end'" in {
      ...
      interval overlapOnlyOnMyEnd passed shouldBe true
    }
