...
class ScheduleSplitTimeSegmentsSpec extends WordSpec with Matchers with ScalaCheckPropertyChecks {
  ...
  "a Schedule with an empty planning" when {
    ...
    "has an empty list of split segments" in {
      emptySchedule.splitTimeSegmentsSingleDate(aThursday) shouldBe Nil
    }
  }

  "a Schedule" when {
    "has a list of split segments" in {
      schedule.splitTimeSegmentsSingleDate(aThursday) shouldBe
        List(
          "10:00" - "12:00",
          "11:00" - "13:00",
          "12:00" - "14:00",
      ...
      schedule.splitTimeSegmentsSingleDate(aTuesday) shouldBe
        List(
          "10:00" - "12:00",
          "11:00" - "13:00",
          "12:00" - "14:00",
  ...
  "a Schedule" when {
    "splits when the day is cut 9-11 11-15" in {
      schedule
        ...
        .splitTimeSegmentsSingleDate(aThursday) shouldBe
  ...
  "a Schedule with exceptions" when {
    ...
    "splits when the day is cut 10-12 13-16" in {
      scheduleWithExceptions.splitTimeSegmentsSingleDate(aThursday) shouldBe
